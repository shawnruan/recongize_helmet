...
def get_output_file_path(output_prefix, model, category, experiment_type):
    """获取输出文件路径
    ...
    if experiment_type == "count":
        # count实验使用数据集名称作为标识符
        return os.path.join(RESULT_DIR, f"{output_prefix}_{clean_model}_{category}_report.json")
    else:
        # crop实验使用类别名作为标识符
        return os.path.join(RESULT_DIR, f"{output_prefix}_{clean_model}_{category}_report.json")
